...
external make: (
  ~ref: ref=?,
  ~onValueChange: ('a, int) => unit=?, // Picker props
  ~selectedValue: 'a=?, // value == string || float
  ~enabled: bool=?,
  ~dropdownIconColor: // Android only
  string=?,
...
module Item = {
  @react.component @bs.module("@react-native-picker/picker") @bs.scope("Picker")
  external make: (
    ~value: 'a=?,
    ~label: string=?,
